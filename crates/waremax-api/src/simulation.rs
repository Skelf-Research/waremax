...
impl ControllableSimulation {
    ...
    pub async fn run(mut self) {
        ...
        self.sim_time_anchor = self.kernel.now().as_seconds();
        self.wall_time_anchor = Instant::now();
        ...
        loop {
            ...
            let wall_elapsed = self.wall_time_anchor.elapsed().as_secs_f64();
            let target_sim_time = self.sim_time_anchor + (wall_elapsed * self.speed);
            let target_time =
    ...
    fn handle_command(&mut self, cmd: SimCommand) -> bool {
        match cmd {
            ...
            SimCommand::SetSpeed(speed) => {
                // Recalibrate time anchors before changing speed to avoid
                // a discontinuity in target simulation time.
                self.sim_time_anchor = self.kernel.now().as_seconds();
                self.wall_time_anchor = Instant::now();
        ...
        match &event.event {
            SimEvent::RobotArriveNode {
                ...
                let old_state = self
                    ...
                    .robots
                    .get(robot_id)
                    .map(|r| format!("{:?}", r.state).split_whitespace().next().unwrap_or("Unknown").to_string());
                ...
                // Emit state change if we can determine the new state after arrival
                if let Some(robot) = self.world.robots.get(robot_id) {
                    let new_state = format!("{:?}", robot.state)
                        .split_whitespace()
                        .next()
                        .unwrap_or("Unknown")
                        .to_string();
                    if let Some(old) = old_state {
                        if old != new_state {
                            let _ = self.update_tx.send(SimUpdate::RobotStateChanged {
                                robot_id: robot_id.0,
                                old_state: old,
                                new_state,
                                time_s: self.kernel.now().as_seconds(),
            ...
            SimEvent::RobotDepartNode { robot_id, .. } => {
                if let Some(robot) = self.world.robots.get(robot_id) {
                    let old_state = format!("{:?}", robot.state)
                        .split_whitespace()
                        .next()
                        .unwrap_or("Unknown")
                        .to_string();
                    let new_state = "Moving".to_string();
            ...
            SimEvent::StationServiceStart { robot_id, .. } => {
                let _ = self.update_tx.send(SimUpdate::RobotStateChanged {
                    robot_id: robot_id.0,
                    old_state: "Moving".to_string(),
                    new_state: "Servicing".to_string(),
                    time_s: self.kernel.now().as_seconds(),
                });
            }
            SimEvent::StationServiceEnd { robot_id, .. } => {
                let _ = self.update_tx.send(SimUpdate::RobotStateChanged {
                    robot_id: robot_id.0,
                    old_state: "Servicing".to_string(),
                    new_state: "Idle".to_string(),
                    time_s: self.kernel.now().as_seconds(),
                });
            }
            SimEvent::RobotChargingStart { robot_id, .. } => {
                let _ = self.update_tx.send(SimUpdate::RobotStateChanged {
                    robot_id: robot_id.0,
                    old_state: "Idle".to_string(),
                    new_state: "Charging".to_string(),
                    time_s: self.kernel.now().as_seconds(),
                });
            }
            SimEvent::RobotChargingEnd { robot_id, .. } => {
                let _ = self.update_tx.send(SimUpdate::RobotStateChanged {
                    robot_id: robot_id.0,
                    old_state: "Charging".to_string(),
                    new_state: "Idle".to_string(),
                    time_s: self.kernel.now().as_seconds(),
                });
            }
            SimEvent::RobotFailure { robot_id, .. } => {
                let _ = self.update_tx.send(SimUpdate::RobotStateChanged {
                    robot_id: robot_id.0,
                    old_state: "Moving".to_string(),
            ...
            SimEvent::RobotPositionUpdate {
                ...
                let _ = self.update_tx.send(SimUpdate::RobotPositionUpdate {
                    robot_id: robot_id.0,
                    edge_id: edge_id.0,
                    from_node: from_node.0,
                    to_node: to_node.0,
            ...
            SimEvent::MetricsSampleTick => {
                let _ = self.update_tx.send(SimUpdate::MetricsUpdate(self.compute_metrics()));
    ...
    fn add_robot(&mut self, node_id: Option<u32>) {
        ...
        let mut robot = Robot::new(new_id, start_node, 1.5, 25.0);
        robot.last_state_change = self.kernel.now();
...
fn build_world_from_config(scenario: &ScenarioConfig, seed: u64) -> World {
    ...
    for row in 0..grid_size {
        for col in 0..grid_size {
            ...
            let node_type = if station_nodes.contains(&id) {
                // Will be overwritten below with actual station type
    ...
    for (idx, station_cfg) in scenario.stations.iter().enumerate() {
        let (station_type, node_type) = match station_cfg.station_type.as_str() {
            "pick" => (StationType::Pick, NodeType::StationPick),
            "drop" => (StationType::Drop, NodeType::StationDrop),
            "inbound" => (StationType::Inbound, NodeType::StationInbound),
            "outbound" => (StationType::Outbound, NodeType::StationOutbound),
            _ => (StationType::Pick, NodeType::StationPick),
        };
        ...
        // Update the map node type to match the actual station type
        if let Some(node) = world.map.nodes.get_mut(&NodeId(node_id)) {
...
///
/// The simulation runs in an isolated task so that a panic in the engine
/// does not crash the API server. On panic, a [`SimUpdate::Error`] is
...
pub fn spawn_simulation(
    ...
    let sim = ControllableSimulation::new(&config, command_rx, update_tx);
    ...
    // Spawn an outer task that monitors the inner simulation task for panics.
    let task = tokio::spawn(async move {
        ...
        match inner.await {
            Ok(()) => {
                tracing::info!("Simulation completed normally");
            }
            Err(e) => {
                let msg = if e.is_panic() {
                    ...
                    tracing::error!(error = %panic_msg, "Simulation panicked");
                    format!("Simulation panicked: {}", panic_msg)
                } else {
                    tracing::warn!("Simulation task was cancelled");
                    "Simulation cancelled".to_string()
                };
                let _ = update_tx_panic.send(SimUpdate::Error(msg));
            }
        }
    });
