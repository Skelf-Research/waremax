...
use waremax_core::{Kernel, SimEvent, SimTime};
use waremax_metrics::{
    BatteryReport, CongestionReport, EdgeCongestion, HeatmapData, MetricsCollector, NodeCongestion,
...
impl SimulationRunner {
    pub fn new(world: World, duration_minutes: f64, warmup_minutes: f64) -> Self {
        Self {
            kernel: Kernel::new(),
            world,
    ...
    pub fn run(&mut self) -> SimulationReport {
        ...
        while self.kernel.has_events() {
            ...
            if let Some(event) = self.kernel.pop_next() {
                ...
                // Handle the event
                self.handler
                    .handle(&mut self.kernel, &mut self.world, &event, &mut self.metrics);
            }
        }
    ...
    fn generate_report(&self) -> SimulationReport {
        ...
        let total_charging_events: u32 =
            self.world.robots.values().map(|r| r.charging_events).sum();
        let total_energy_consumed: f64 = self
            .world
            .robots
            .values()
            .map(|r| r.total_energy_consumed_wh)
            .sum();
        let total_charging_time: f64 = self
            .world
            .robots
            .values()
    ...
    pub fn generate_robot_reports(&self, total_time_s: f64) -> Vec<RobotReport> {
        self.world
            ...
            .map(|robot| {
                let working_time =
                    robot.total_move_time.as_seconds() + robot.total_service_time.as_seconds();
                ...
                let idle_time = total_time_s
                    - working_time
                    - robot.total_charging_time.as_seconds()
                    - maintenance_time;
    ...
    pub fn generate_heatmap(&self) -> HeatmapData {
        ...
        // Node congestion
        let node_congestion: Vec<NodeCongestion> = ts
            .node_congestion
            .iter()
            .map(|(node_id, metrics)| {
                let (x, y) = self
        ...
        // Edge congestion
        let edge_congestion: Vec<EdgeCongestion> = ts
            .edge_congestion
            .iter()
            .map(|(edge_id, metrics)| {
                let (from, to) = self
