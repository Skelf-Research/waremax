...
use waremax_core::{SimTime, SimEvent, ScheduledEvent, Kernel, OrderId, TaskId, ChargingStationId, MaintenanceStationId, RobotId};
use waremax_entities::{Order, OrderLine, Task, BinLocation, RobotState};
...
impl EventHandler {
    ...
    pub fn handle(&mut self, kernel: &mut Kernel, world: &mut World, event: &ScheduledEvent, metrics: &mut MetricsCollector) {
        ...
        match &event.event {
            ...
            SimEvent::RobotDepartNode { robot_id, from_node, to_node, edge_id } => {
                self.handle_robot_depart(kernel, world, current_time, *robot_id, *from_node, *to_node, *edge_id, metrics);
            }
            ...
            SimEvent::RobotChargingStart { robot_id, station_id } => {
                self.handle_robot_charging_start(kernel, world, current_time, *robot_id, *station_id, metrics);
            }
            SimEvent::RobotChargingEnd { robot_id, station_id, energy_charged_wh } => {
                self.handle_robot_charging_end(kernel, world, current_time, *robot_id, *station_id, *energy_charged_wh);
            }
            ...
            SimEvent::DeadlockResolved { robots, resolver_robot } => {
                self.handle_deadlock_resolved(kernel, world, *resolver_robot, robots.clone());
            }
            // v3: Robot Failures & Maintenance
            SimEvent::RobotFailure { robot_id, interrupted_task } => {
                self.handle_robot_failure(kernel, world, current_time, *robot_id, *interrupted_task, metrics);
            }
            SimEvent::RobotMaintenanceDue { robot_id, operating_hours: _ } => {
                self.handle_robot_maintenance_due(kernel, world, current_time, *robot_id);
            }
            SimEvent::MaintenanceStart { robot_id, station_id, is_repair } => {
                self.handle_maintenance_start(kernel, world, current_time, *robot_id, *station_id, *is_repair, metrics);
            }
            SimEvent::MaintenanceEnd { robot_id, station_id, is_repair, duration_s } => {
                self.handle_maintenance_end(kernel, world, current_time, *robot_id, *station_id, *is_repair, *duration_s, metrics);
            }
            _ => {
    ...
    fn handle_order_arrival(
        ...
        for (task_id, sku_id, quantity, bin_location, dest_station) in tasks_to_create {
            ...
            if world.attribution_collector.is_enabled() {
                world.attribution_collector.start_task(task_id, Some(order_id), current_time);
                world.attribution_collector.start_phase(task_id, DelayCategory::RobotAssignment, current_time);
            }
        }
    ...
    fn handle_task_assignment(
        ...
        if world.attribution_collector.is_enabled() {
            world.attribution_collector.record_robot_assignment(task_id, robot_id, current_time);
            world.attribution_collector.start_phase(task_id, DelayCategory::TravelToPickup, current_time);
        }
            ...
            if let Some(route) = world.router.find_route(&world.map, robot_node, pickup_node) {
                ...
                if world.reservation_manager.enabled {
                    ...
                    for window in route.path.windows(2) {
                        ...
                        if let Some((_, edge_id, length)) = world.map.neighbors(from_node)
    ...
    fn handle_robot_depart(
        ...
        if !world.traffic.can_enter_edge(edge_id, robot_id) {
            ...
            world.time_series.record_edge_wait(edge_id, SimTime::from_seconds(0.5));
        ...
        let edge_length = world.map.get_edge(edge_id).map(|e| e.length_m).unwrap_or(0.0);

        // Update robot state and consume battery
        if let Some(robot) = world.get_robot_mut(robot_id) {
            robot.state = RobotState::Moving { destination: to_node };
                ...
                if let Some((source_node, destination_station)) = task_info {
                    if node_id == source_node {
                        ...
                        if world.attribution_collector.is_enabled() {
                            ...
                            world.attribution_collector.start_phase(task_id, DelayCategory::TravelToStation, current_time);
                        }
                        ...
                        if let Some(robot) = world.get_robot(robot_id) {
                            if let Some(next) = robot.next_node_in_path() {
                                ...
                                if world.attribution_collector.is_enabled() {
                                    world.attribution_collector.start_phase(task_id, DelayCategory::StationQueue, current_time);
                                }
                                kernel.schedule_now(SimEvent::StationServiceStart {
                                    robot_id,
    ...
    fn handle_service_end(
        ...
        if world.attribution_collector.is_enabled() {
            world.attribution_collector.complete_task(task_id, current_time);
        }
    ...
    fn handle_metrics_sample_tick(
        ...
        for (station_id, station) in &world.stations {
            world.time_series.record_station_queue(*station_id, current_time, station.queue_length());

            // Calculate current utilization
            let utilization = station.serving_count() as f64 / station.concurrency.max(1) as f64;
            world.time_series.record_station_utilization(*station_id, current_time, utilization);
        }

        // Sample charging station states
        for (station_id, station) in &world.charging_stations {
            world.time_series.record_charging_queue(*station_id, current_time, station.queue.len());
            world.time_series.record_charging_bays(*station_id, current_time, station.charging.len());

            let utilization = station.charging.len() as f64 / station.bays.max(1) as f64;
            world.time_series.record_charging_utilization(*station_id, current_time, utilization);
        }
    ...
    fn handle_robot_low_battery(
        ...
        if let Some(from_node) = robot_node {
            if let Some(charging_station_id) = world.find_nearest_charging_station(from_node) {
                ...
                let station_node = world.get_charging_station(charging_station_id).map(|s| s.node);
                ...
                if let Some(dest_node) = station_node {
                    if let Some(route) = world.router.find_route(&world.map, from_node, dest_node) {
                        if let Some(robot) = world.get_robot_mut(robot_id) {
                            ...
                            robot.state = RobotState::SeekingCharge { destination: charging_station_id };
                        }
    ...
    fn handle_robot_charging_start(
        ...
        let has_free_bay = world.get_charging_station(station_id)
        ...
        if has_free_bay {
            ...
            // Update robot state
            if let Some(robot) = world.get_robot_mut(robot_id) {
                robot.state = RobotState::Charging { at_station: station_id };
                robot.update_stats(current_time);
            }
            ...
            let (charge_duration, energy_charged_wh, soc_for_trace) = {
                ...
                if let (Some(robot), Some(station)) = (robot, station) {
                    ...
                    if battery.enabled {
                        ...
                        let duration = station.charging_duration(battery.soc, 1.0, battery.capacity_wh);
            ...
            kernel.schedule_after(charge_duration, SimEvent::RobotChargingEnd {
                robot_id,
                station_id,
                energy_charged_wh,
            });
    ...
    fn handle_robot_charging_end(
        ...
        let charging_duration = {
            world.get_charging_station(station_id)
                .and_then(|s| {
                    s.charging.iter()
                        .find(|(r, _)| *r == robot_id)
        ...
        let next_robot = world.get_charging_station_mut(station_id)
            ...
            if let Some(station_id) = world.find_nearest_maintenance_station(from_node) {
                ...
                if let Some(dest_node) = station_node {
                    if let Some(route) = world.router.find_route(&world.map, from_node, dest_node) {
                        ...
                        if route.path.len() > 1 {
                            // Find edge between nodes
                            if let Some((_, edge_id, _)) = world.map.neighbors(route.path[0])
    ...
    fn handle_robot_maintenance_due(
        ...
        let (is_available, from_node, seeking) = {
            world.get_robot(robot_id)
                .map(|r| (r.is_idle() && r.current_task.is_none(), r.current_node, r.seeking_maintenance))
    ...
    fn handle_maintenance_end(
        ...
        if let Some(next_robot_id) = next_robot_info {
            ...
            let next_is_repair = world.get_robot(next_robot_id)
                .map(|r| r.is_failed())
                .unwrap_or(false);
